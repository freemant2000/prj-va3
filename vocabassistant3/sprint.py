...
class ExeciseWord(Base):
    ...
    def get_meanings(self)->str:
        return self.wd.get_selected_meanings(WordDef.get_int_m_indice(self.m_indice))
...
class ExerciseDraft:
    ...
    used_snt_cands: List[Sentence] = field(default_factory=list)
...
def refine_exec_draft(s: Session, sp: Sprint, ed: ExerciseDraft):
    ...
    ed.extra_kws.clear()
    ed.used_sds.clear()
    ed.used_snt_cands.clear()
    ed.used_l2_snt_cands.clear()
    for snt in ed.snt_cands:
         if sp.uses_snt(snt):
            ed.used_snt_cands.append(snt)
    for sd in ed.sds:
        if sd.snt_id!=None:
            snt=get_snt(s, sd.snt_id)
